...
## PART 1: THE NOTES (Explained in Full Detail)
...
All 4 PCs are physically wired into the switch.
Any of them can instantly send data to any other one.
...
PC1---+
PC2---+--[ SWITCH ]---[ ROUTER ]---[ ISP ]---[ THE INTERNET ]
PC3---+
PC4---+

If PC1 wants to reach a website on the internet, here's
the exact order the data travels through:
...
(public — shared with EVERYONE)
     ...
     All three offices ALSO run their own private,
     secured connection riding on top of that same
     public internet — accessible ONLY to this one
...
YOUR HOUSE
|
[ LOCAL ISP ] — connects individual neighborhoods
|
[ REGIONAL ISP ] — connects multiple cities within a country
|
[ GLOBAL ISP ] — connects entire countries together (the "backbone")
|
...
## PART 2: MY 9 SELF-QUIZ QUESTIONS AND ANSWERS
...
↓
HOME ROUTER (this is actually a switch + router combined into one box)
|
| (router forwards it out)
↓
THE INTERNET (made up of an enormous number of other interconnected routers)
|
| (the packet automatically chooses the fastest,
| most convenient available path to take)
↓
GOOGLE'S SERVER
|
| (Google's server receives the packet, checks
| exactly what was requested, and processes it)
↓
| (sends a response back, retracing a similar path)
↓
THE INTERNET
|
↓
...
LCB ------------↘-----------CM-----------↙------------ RCB
(every single player
on the team passes the
ball back through CM
before it goes to
anyone else)
= THE SWITCH
LB RB
       ...
       THIS ENTIRE HALF OF THE FIELD
                = A LAN
    (Team A's own side — everyone here
     is close, connected, and can pass
     freely without ever needing ST)
...
### PART 1: WHAT I LEARNED (Full Notes)
...
Authoritative Server → gives the REAL final IP address
    ↓
Resolver saves it in cache, sends it back to browser
    ↓
...
- "set Radius to 0" = declaring a variable → `let radius = 0;`
- "repeat until Radius > 40" = a while loop → `while (radius <= 40) { ... }`
- "change Radius by 0.2" = incrementing a variable → `radius += 0.2;`
- "change Radius by -0.2" = decrementing a variable → `radius -= 0.2;`
...
- Adding images
- Adding emails
- And even linking websites in our websites
...
- **ID:** "This is this specific element."
...
<div> <header>James Bound</header> <p>...</p> <ul><li>...</li></ul> </div>

Code 2:

<h2>...</h2> <p>...</p> <ol><li>...</li></ol> ```` Div tells the browser: "hey bro, these codes are separate." It puts the whole Code 1 in a container, and anyone viewing that webpage can easily know: "hey, these are two different parts and don't go together." That's the work of div.

Q3: You wrote that a class is like saying "everything in this group
should have the same styling." Can you explain WHY reusability matters —
what would go wrong if every single element needed its own separate CSS
rule instead?

Reusability matters so much because of styling. If you have a thousand
lines of code, and hundreds of elements that you would want to have the
same styling — let's say you want every list in your code to have the
same CSS — the reasonable thing to do is to assign every single one of
those elements the same class name. So when you want to style those
elements, rather than styling each and every one of them one at a time,
you can do them once, by just calling the assigned name which every one
of those elements has, as they have been classed and assigned a name,
which can be reused a thousand times in the same project. So it makes
styling easy, fast, and moreover it keeps our codes in an organised way,
also making it readable.

If every single element needed its own separate CSS rule instead: if it's
a huge project, let's say you're to style a thousand elements under 10
minutes — if you give every one of them a class and assign them a name,
with one call each, every one of those elements gets styled. But without
it, you will spend hours doing one at a time, and may also skip or miss
some elements unstyled. It will take a toll on the code, and make the
code disorganised, especially at scale.

Q4: What is the actual difference between how you'd use class="btn"
vs id="login-button" on the exact same button — could you use both at
once, and if so, why would you?

To my own understanding, I would use class="btn" AND id="login-button"
on the same button if I want that button to carry the same styling as
every other button on that website (via the shared class), but also want
it to be styled separately/uniquely, or be targetable individually (via
the unique ID).

Let's say I'm working on a fintech website project. It will have numerous
buttons, but will only have a single login button and a single sign-up
button — that's where id="login-button" and id="signup-button" come
in. But if I want other buttons like home button, submit button, profile
button, and all that to have the same styling, I can use class="btn" —
meaning every button that has been categorized under that class="btn"
will have the same styling.

Confirmed: yes, you CAN use both at once on the same button — this is
actually standard practice:

html
<button id="login-button" class="btn">Login</button>

The class gives shared styling (padding, border-radius, same as every
other button), while the ID lets you target THIS specific button for
something unique — like a special colour, or hooking it up to JavaScript
specifically.

Q5: Why should you NEVER give two different elements the same ID on
one page — what could actually break (technically) if you did?

I honestly didn't know this one, so rather than guess, I researched it
properly instead of wasting time faking an answer.

What I learned: Two real, technical things break:

JavaScript gets confused. When code uses
document.getElementById("something"), it's designed to return exactly
ONE element. If two elements share that same ID, JavaScript can only grab
the FIRST one it finds, completely ignoring the second. So if code tried
to make a button do something using its ID, and another element
accidentally shared that ID, the code might silently target the wrong
element — or seem to do nothing at all.
It breaks the fundamental promise an ID makes. The entire meaning of
an ID is "this is the ONE unique thing." If two elements share an ID,
that promise is broken — this also technically makes the HTML invalid
according to web standards, even if the browser still tries to render it.

Simple way to remember it: an ID is like a national ID number — if two
different people had the exact same ID number, any system trying to look
someone up by that number wouldn't know which person is meant. That's the
same chaos duplicate IDs cause in code.

Q6: Using the house analogy you built — if HTML is structure and CSS
is appearance, explain what would happen if a website had HTML and CSS
but NO JavaScript at all. What would still work, and what wouldn't?

If a website had HTML and CSS but no JavaScript at all — yes, you will be
able to view the website, see it, and admire it. You as a user won't be
able to do anything on that website, as it would be dysfunctional.

Imagine you have a house, with roof, doors, and all that beautifully
painted, but nothing works — nothing happens when you turn on the light
switch to turn on the heater. Nothing happens.

That's exactly a website without JavaScript. It doesn't function. It
reads that website, one view-only website, no matter how you click
signup or login, no matter how you click those buttons, nothing happens.
Exactly a website without JavaScript — you can see the well laid out
sign-up form but can't submit. You can see the login form but can't
click. On your profile icon to do your changes — that website becomes
unfunctional.

Struggled with:

Q5 was genuinely something I didn't know the answer to. Rather than
guessing or making something up to fill the space, I chose to be honest
and actually research it properly. I now understand it's not just "bad
practice" — it's a real technical problem, since JavaScript's
getElementById is designed to return exactly one element, and duplicate
IDs break that guarantee.

I also wasn't sure whether class and ID could be used together on the
same element — confirmed today that yes, this is actually standard
practice, not an either/or choice.

Tomorrow:

Not sure yet what's coming next in class — likely more HTML (semantic
tags, forms, or moving into CSS properly). Will bring today's
...
![Raffle ticket assignment code](./images/day6-7-math-random-assignment.png)
...
// Step 2: Calculate the winning ticket number
...
199 + 1 = 200                 // ✅ Valid, matches an existing ticket
```
...
Once this pattern clicks, it applies far beyond raffles — dice rolls, random level selection, lottery draws, and more.

---

## 🧠 Self-Quiz (My Answers, Confirmed Correct)

**Q1: Why can't we just use `Math.random()` by itself, with no multiplication?**
A: `Math.random()` only ever produces a decimal below 1. Without multiplying by the total pool size, there's no way to scale that decimal into a usable whole number range. The result would always stay below 1.

**Q2: What goes wrong if we forget the `+1`?**
A: Without `+1`, the range becomes `0` to `totalTickets - 1` — one student gets "ticket 0" (which doesn't exist), and nobody ever gets the highest number. `+1` shifts the whole range up by one so every value maps to a real ticket.

**Q3: What goes wrong if we use `Math.ceil` instead of `Math.floor`?**
A: `Math.ceil` rounds up. In a 200-student raffle, if `Math.random()` returns `0.998`, `0.998 * 200 = 199.6`, and `Math.ceil` rounds that up to `200`. Adding `1` gives `201` — an impossible ticket. `Math.floor` avoids this by rounding down to `199`, giving `200` — a valid ticket.

**Q4: Why doesn't changing `totalTickets` from 13 to 50 require touching the `Math.floor(...)` line?**
...
The real boundary: `Math.floor(Math.random() * N)` produces any whole number from `0` to `N - 1`. Adding `1` shifts that whole set up by one, losing nothing at either end.
